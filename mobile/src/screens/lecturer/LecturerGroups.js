...
const LecturerGroups = ({ user, onLogout }) => {
        ...
        { text: 'Cancel', style: 'cancel' },
        {
          ...
          onPress: async () => {
            try {
              ...
              setGroupMembers(members);
              // Reload groups to update leader info
              loadGroups();
            } catch (error) {
  ...
  const renderGroupItem = ({ item }) => (
    ...
    >
        ...
        <View style={[
          styles.statusBadge,
          { backgroundColor: item.status === 'active' ? '#52c41a15' : '#99915' }
        ]}>
          <Text style={[
            styles.statusText,
            { color: item.status === 'active' ? '#52c41a' : '#999' }
          ]}>
    ...
    return (
      <View style={styles.memberCard}>
        ...
        <View style={styles.memberFooter}>
          <View style={[
            styles.roleBadge,
            { backgroundColor: isLeader ? '#ffc10715' : '#667eea15' }
          ]}>
            <Text style={[
              styles.roleText,
              { color: isLeader ? '#ffc107' : '#667eea' }
            ]}>
              {item.roles?.toUpperCase() || 'MEMBER'}
            </Text>
          </View>
            {selectedGroup && (
